...
tunelConRampita  tiro 
 | (condicionParaSuperarloEnPrecision (>90) tiro) && (condicionParaSuperarloEnAltura (==0) tiro) = modificarVelocidadyPrecision 2 tiro
...
modificarVelocidadyPrecision :: Float -> Tiro -> Tiro
modificarVelocidadyPrecision n tiro = UnTiro {velocidad = (velocidad tiro) * n , precision = 100, altura = 0}
...
laguna largoLaguna tiro
  | (condicionParaSuperarloEnVelocidad (>80) tiro) && (condicionParaSuperarloEnAltura (<5) tiro) =  (efectoLaguna largoLaguna) tiro
  | otherwise = tiroDetenido
